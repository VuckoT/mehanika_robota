...
M = kkt.SE3_sastavi(np.eye(3), [L1 + L2, 0, 0])

theta = np.array([np.pi/6, -np.pi/3], dtype=float)

Td = kkt.exp_vek_ugao(S1, theta[0]) @ kkt.exp_vek_ugao(S2, theta[1]) @ M
T1 = Td @ kkt.inv(M)
...
Phi[0] = kin.paden_kahan1(S1, r2, kkt.SE3_proizvod_3D(T1, r2))
Phi[1] = kin.paden_kahan1(S2, p2, kkt.SE3_proizvod_3D(
    kkt.exp_vek_ugao(S1, -Phi[0])@T1,
    p2
))

Ts = kkt.exp_vek_ugao(S1, Phi[0]) @ kkt.exp_vek_ugao(S2, Phi[1]) @ M
